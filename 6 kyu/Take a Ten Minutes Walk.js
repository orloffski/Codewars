...
const isValidWalk = (walk) => {
	...
	let coords = new Map([['n', 's'], ['s', 'n'], ['e', 'w'], ['w', 'e']]);
	let path = new Map();
	let result = true;

	walk.forEach(element => {
		if(path.has(element)){
			path.set(element, path.get(element) + 1);
		}else{
			path.set(element, 1);
		}
	});

	for (let side of coords.keys()){
		if(path.has(side)){
			if(!path.has(coords.get(side)) || path.get(side) != path.get(coords.get(side))){
				result = false;
			}
		}
	}
